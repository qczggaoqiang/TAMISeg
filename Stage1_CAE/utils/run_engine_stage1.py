...
def load_names(path, file_path):
    with open(file_path, 'r') as r:
        data = [r_str.replace('\n', '') for r_str in r.readlines()]
    images = [os.path.join(path, "frames", name + ".png") for name in data]
...
def load_data(path, txt_name_path=None, train_flag=True):

    if train_flag:
        train_x, train_y = load_names(os.path.join(path, 'train'), txt_name_path)
    else:
        train_x, train_y = load_names(os.path.join(path, 'test'), txt_name_path)
...
class DATASET(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if os.path.exists(image_path) is False:
            image_name = image_name.replace('mask_', '')
        ...
        if self.transform is not None:
            ...
            augmentations_bg = self.transform(image=image, mask=background)
            background = augmentations_bg["mask"]
...
def train(model, loader, optimizer, loss_fn, device, consistency_loss_fn=BinaryConsistencyLoss()):
    ...
    augmentations = transforms.Compose([
        ...
        transforms.RandomErasing(p=0.4, scale=(0.02, 0.1), ratio=(0.3, 3.3))
    ])

    train_tqdm = tqdm(loader, total = len(loader))
    for i, ((x), (y1, y2)) in enumerate(train_tqdm):
        ...
        train_tqdm.set_postfix(iter = i,
                                loss_mask = loss_mask.item(),
                                loss_mask_aug = loss_mask_aug.item(),
