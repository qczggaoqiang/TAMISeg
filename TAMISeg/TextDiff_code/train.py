...
def evaluation(args, model, extractor, valloader):
    ...
    # for idx in tqdm(list(range(10)), desc='val:'):
        ...
        ##########
        mask = cv2.imread(os.path.join("datasets/Covid19/test/masks", sample["name"] + '.png'), 0)
        mask[mask <= 0] = 0
        mask[mask > 0] = 1
        ###########
...
if __name__ == '__main__':
    ...
    train_text = read_text(os.path.join(opts['training_path'], 'Train_text_MosMedData.xlsx'))
    ...
    val_text = read_text(os.path.join(opts['validation_path'], 'Test_text_MosMedData.xlsx'))
