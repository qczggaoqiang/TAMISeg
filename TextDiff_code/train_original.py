...
def evaluation(args, model, extractor, valloader):
    ...
    # for idx in tqdm(list(range(10)), desc='val:'):
        ...
        features = collect_features_condseg(features)#condseg
        ...
        with torch.no_grad():
            ...
            pred = F.interpolate(pred, size=(args["image_size"], args["image_size"]), mode='bilinear',
                                 align_corners=False)
            assert pred.dim() == 4 and pred.shape[1] > 1, "pred outputs should have >1 classes"
            pred_softmax = torch.softmax(pred, dim=1)
            #_, pred = torch.max(pred_softmax, dim=1)
            pred = torch.argmax(pred_softmax, dim=1)  # [B,H,W]
            pred = pred.squeeze(0).cpu().numpy().astype(np.uint8)  # [H,W]

        ##########
        #mask = cv2.imread(os.path.join("datasets/QaTa-COV19-v2/test/masks", sample["name"] + '.png'), 0)
        #mask[mask <= 0] = 0
        #mask[mask > 0] = 1
        ###########

        mask_dir = "datasets/KSeg/test/masks"
        #mask_name = "mask_" + sample["name"] + ".png"  # 🔹 这里加了 mask_ 前缀
        mask_name = sample["name"] + ".png"  # 🔹 这里加了 mask_ 前缀
        mask_path = os.path.join(mask_dir, mask_name)
        mask = cv2.imread(mask_path, cv2.IMREAD_GRAYSCALE)

        if mask is None:  # 🔹 增加健壮性检查
...
def main(args, extractor, data_loader):
    ...
    extract_dims = [1024, 512, 256, 64]  #condseg
    ...
    for epoch in range(args['max_training']):
        ...
        # for idx, sample in tqdm(list(enumerate(data_loader))):
        #     img, label, text, name = sample['image'], sample['label'], sample['text'], sample['name']
