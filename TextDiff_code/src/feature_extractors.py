...
def create_feature_extractor(model_type, **kwargs):
    ...
    if model_type == 'ddpm':
        ...
        print("Creating ConDSeg-stage1 Feature Extractor...")
        feature_extractor = FeatureExtractorConDSeg(**kwargs)
    else:
...
class FeatureExtractorConDSeg(FeatureExtractor):
    ''' 
    ...
    def _load_pretrained_model(self, model_path, **kwargs):
        ...
        self.model = ConDSegStage1()
        ckpt = self.load_state_dict(model_path, map_location="cpu")
        current_ckpt = self.model.state_dict()
        ckpt = {k: v for k, v in ckpt.items() if k in current_ckpt}
...
class FeatureExtractorDDPM(FeatureExtractor):
    ...
    def _load_pretrained_model(self, model_path, **kwargs):
        import inspect
        from guided_diffusion import create_model_and_diffusion
...
def collect_features_condseg(feature_list):
    """ Upsample activations and concatenate them to form a feature tensor """
    
    feats = {}
    for k in range(4):
        feat = feature_list[::-1][k]
        # if feat.shape[-1] == 256:
        #     feats[k] = nn.functional.interpolate(feat, size=(128, 128))
        # else:
        feats[k] = feat
...
def collect_features(activations: List[torch.Tensor], sample_idx=0):
    ...
    feats = {}
    for k in range(4):
        feat = torch.cat(activations[k:12:4], dim=1)
        # if feat.shape[-1] == 256:
        #     feats[k] = nn.functional.interpolate(feat, size=(128, 128))
        # else:
        feats[k] = feat
